...
const v1 = await store.put("eric", { content: "hello", meta: { source: "test" } });
assert(v1.version === 1 && v1.content === "hello", "first put is version 1");

const v2 = await store.put("eric", { content: "world" });
assert(v2.version === 2, "second put increments version");

const got = await store.get("eric");
assert(got?.content === "world" && got.version === 2, "get returns latest pack");
