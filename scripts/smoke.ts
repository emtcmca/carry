...
// --- auth ---
const ns = loadNamespaces('[{"namespace":"eric","readToken":"r_tok","writeToken":"w_tok"}]');
...
assert(hasScope({ namespace: "eric", scope: "write" }, "read"), "write scope satisfies read");
assert(!hasScope({ namespace: "eric", scope: "read" }, "write"), "read scope does not satisfy write");
...
await store.put("eric", "voice", { content: "voice pack" });
const packs = await store.list("eric");
assert(packs.length === 2 && packs.includes("default") && packs.includes("voice"), "list returns named packs");
assert((await store.get("eric", "voice"))?.content === "voice pack", "named pack round-trips");
