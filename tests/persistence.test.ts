...
describe("LibSqlStore durability across instances", () => {
  ...
  it("auto-creates a missing parent directory and persists across a reopen", async () => {
    ...
    await writer.init();
    const put = await writer.put("eric", "default", {
      content: "# Voice\nNo em-dashes.",
    ...
    await reader.init();
    const got = await reader.get("eric", "default");
  ...
  it("keeps the incremented version across a reopen", async () => {
    ...
    await a.init();
    await a.put("eric", "default", { content: "second write" });
    await a.close();

    const b = new LibSqlStore(DB_URL);
    await b.init();
    const got = await b.get("eric", "default");
  ...
  it("persists distinct named packs independently across a reopen", async () => {
    ...
    const writer = new LibSqlStore(DB_URL);
    await writer.init();
    await writer.put("eric", "voice", { content: "voice-only body" });
    await writer.close();

    const reader = new LibSqlStore(DB_URL);
    await reader.init();
    // Both packs survive and are independently versioned.
    expect((await reader.get("eric", "voice"))?.content).toBe("voice-only body");
    expect((await reader.get("eric", "voice"))?.version).toBe(1);
    expect((await reader.get("eric", "default"))?.version).toBe(2);
    expect((await reader.list("eric")).sort()).toEqual(["default", "voice"]);
    await reader.close();
  });
});
